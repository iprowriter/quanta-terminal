"""
Research Agent
LangGraph agent that analyses arXiv research papers, publication velocity, and technology positioning.

Tools: search_papers · get_papers_for_ticker · get_paper_detail · get_sector_research_pulse
...
from prompts.system_prompts import research_system_prompt, ticker_hint
...
_TECH_TOOL_NAMES = {
    "search_papers",
    "get_papers_for_ticker",
    "get_paper_detail",
    "get_sector_research_pulse",
}
...
async def llm_call(state: TechState, model_with_tools) -> dict:
    ...
    system = SystemMessage(content=research_system_prompt + ticker_hint(state["messages"]))
    response = await model_with_tools.ainvoke([system] + state["messages"])
    return {"messages": [response]}


async def tool_node(state: TechState, tools_by_name: dict[str, BaseTool]) -> dict:
    ...
    for tc in tool_calls:
        if tc["name"] in ("get_papers_for_ticker", "search_papers"):
            cache_updates["papers"] = tc["args"]
        elif tc["name"] == "get_sector_research_pulse":
            cache_updates["sector_pulse"] = tc["args"]

    return {"messages": list(tool_messages), **cache_updates}


def should_continue(state: TechState) -> str:
...
async def initialize() -> None:
    ...
    _mcp_client = MultiServerMCPClient(
        {
            "research": {
                ...
                "args": [str(MCP_DIR / "research_mcp.py")],
                "transport": "stdio",
                "env": _mcp_env(),
            }
        }
    ...
    tools = [t for t in all_tools if t.name in _TECH_TOOL_NAMES]
    tools_by_name = {t.name: t for t in tools}

    model = _build_model()
    model_with_tools = model.bind_tools(tools)

    async def _llm_call(state: TechState):
        return await llm_call(state, model_with_tools)

    async def _tool_node(state: TechState):
...
async def generate_response(
    ...
    """
    Run the tech agent and return a structured result.

    Returns:
        {
            "text":         str  — the agent's final response,
            "thread_id":    str  — pass back to continue this conversation,
            "papers":       dict — cached papers output,
            "sector_pulse": dict — cached get_sector_research_pulse output,
        }
    """
    if _graph is None:
        raise RuntimeError("Tech agent not initialised — call initialize() first.")

    thread_id = thread_id or str(uuid.uuid4())

    initial_state: TechState = {
    ...
    return {
        ...
        "sector_pulse": result.get("sector_pulse", {}),
    }
...
if __name__ == "__main__":
    async def _main() -> None:
        await initialize()
        print("Tech Agent ready. Type 'quit' to exit, 'new' to start a fresh thread.\n")
        thread_id: str | None = None
